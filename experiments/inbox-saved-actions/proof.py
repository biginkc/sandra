...
if not TEST.strip().startswith('DO $test$') or 'ALL SAVED-ACTION SQL PROOFS PASSED' not in TEST:
    raise RuntimeError('Unexpected proof scenario source')
output = sql_with_notices('BEGIN;' + TEST + 'ROLLBACK;')
if 'ALL SAVED-ACTION SQL PROOFS PASSED' not in output:
    raise RuntimeError(f'Proof scenario did not report success: {output}')
PASS_LINES = [line for line in output.splitlines() if 'PASS ' in line]
if len(PASS_LINES) < 13:
    raise RuntimeError(f'Expected at least 13 PASS lines, got {len(PASS_LINES)}: {output}')
