...
if sys.argv[1:]:
 ...
 for f in P.glob('*.sql'):
  for name,body in re.findall(r'CREATE (?:OR REPLACE )?FUNCTION ([\w.]+)\(.*?AS \$\$(.*?)\$\$;',f.read_text(),re.S):
   if installed.get(name,'').strip()!=body.strip():raise RuntimeError('Installed body differs: '+name)
   count+=1
 print(f'{count} installed outcome function bodies match reviewed source')
