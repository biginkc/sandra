...
- `GET /api/inbox/operations/recover?idempotencyKey=...`
...
SMS preparation review reports deduplicated contact, linked-property and active-enrollment counts from the immutable prepared scope, including affected unselected sibling properties. Recovery is requester/org scoped to one original key. Null means not observed at that read; an already in-flight acceptance may still commit. The client must retain/retry the same preparation/key while uncertain, never invent a fresh key from a null lookup. The optional approved browser recovery reference contains only opaque preparation/key under auth identity; no selection, message content, PII or localStorage.
