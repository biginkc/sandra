...
functions={}
for path in [P/'setup.sql',P/'worker.sql',P/'accept.sql',P/'public-api.sql',P/'review.sql']:
 for name,body in re.findall(r'CREATE (?:OR REPLACE )?FUNCTION ([a-z_][a-z_0-9]*\.[a-z_][a-z_0-9]*)\([^;]*?AS \$\$(.*?)\$\$;',path.read_text(),re.S):functions[name]=body
...
def execute(op,s):
 first=json.loads(sql(f"SELECT inbox_action_api.run_step('{o}','{op}','{s}')"))
 replay=json.loads(sql(f"SELECT inbox_action_api.run_step('{o}','{op}','{s}')"))
 need(first==replay,'Durable step receipt replay changed')
...
sibling,seq,enrollment=uid(),uid(),uid()
sql(f"INSERT INTO properties(id,org_id,address,state,homeowner_contact_id) VALUES('{sibling}','{o}','Unselected SMS sibling','MO','{c}');INSERT INTO sequences(id,org_id,name) VALUES('{seq}','{o}','Authoritative review scope');INSERT INTO sequence_enrollments(id,org_id,sequence_id,property_id,status) VALUES('{enrollment}','{o}','{seq}','{sibling}','active')")
prep5=prepare([{'type':'outcome','value':'opted_out'}]);need(prep5['sms_safety_summary']=={'contacts':1,'linked_properties':2,'active_enrollments':1},'SMS review scope wrong');op5=accept(prep5)['operation_id'];execute(op5,status(op5)['steps'][0]['id'])
...
need(sql(f"SELECT count(*) FROM inbox_operation_domain.target_versions WHERE org_id='{o}' AND conversation_id='{missing}'")=='0','Absent target created persistent counter')
checks.append('arbitrary absent target is excluded without persistent counter allocation')
