-- Private candidate; no caller or installation in setup.sql yet.
...
BEGIN;
CREATE FUNCTION inbox_operation_domain.apply_sms_opt_out(o uuid,p uuid,actor uuid,s uuid,expected_scope jsonb,expected_policy jsonb) RETURNS jsonb
LANGUAGE plpgsql SET search_path='' AS $$
DECLARE homeowner uuid;scope_revision bigint;contact public.contacts;property_ids uuid[];enrollment_ids uuid[];
 requirements jsonb;actual jsonb;requirement jsonb;consent_id uuid;paused jsonb:='[]';item record;contact_changed boolean:=false;
BEGIN
 ...
 IF homeowner IS NULL THEN RETURN jsonb_build_object('contact_id',NULL,'paused',paused);END IF;
 ...
 IF NOT FOUND OR scope_revision::text IS DISTINCT FROM expected_scope->>'revision' THEN RAISE EXCEPTION 'SMS scope changed or unseeded';END IF;
 ...
 END LOOP;
 RETURN jsonb_build_object('contact_id',homeowner,'contact_changed',contact_changed,'consent_id',consent_id,'paused',paused,
...
END $$;
REVOKE ALL ON FUNCTION inbox_operation_domain.apply_sms_opt_out(uuid,uuid,uuid,uuid,jsonb,jsonb) FROM PUBLIC,anon,authenticated,service_role;
COMMIT;
