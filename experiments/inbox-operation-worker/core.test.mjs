...
test('release HTTP fixture requires the exact target, launcher marker, labels, and constrained login',()=>{
 const env={NODE_ENV:'test',INBOX_ACTION_LOCAL_FIXTURE:'1',INBOX_ACTION_FIXTURE_PROFILE:'release-http',INBOX_ACTION_OWNED_FIXTURE_PLAINTEXT:'true',INBOX_ACTION_FIXTURE_MARKER:'sandra-inbox-http-owned-synthetic-20260917',INBOX_ACTION_FIXTURE_OWNER:'release-infra',INBOX_ACTION_FIXTURE_PURPOSE:'sandra-inbox-release-http',INBOX_ACTION_FIXTURE_LABELS_VERIFIED:'true',INBOX_ACTION_DATABASE_URL:'postgres://inbox_action_worker:synthetic@127.0.0.1:54322/postgres'};
 const config=databaseConfiguration(env);assert.equal(config.port,54322);assert.equal(config.ssl,false);assert.equal(config.user,'inbox_action_worker');
 for(const patch of [{INBOX_ACTION_FIXTURE_MARKER:'wrong'},{INBOX_ACTION_FIXTURE_OWNER:'postgres'},{INBOX_ACTION_FIXTURE_PURPOSE:'shared'},{INBOX_ACTION_FIXTURE_LABELS_VERIFIED:'false'},{INBOX_ACTION_OWNED_FIXTURE_PLAINTEXT:'false'},{INBOX_ACTION_DATABASE_URL:'postgres://inbox_action_worker:synthetic@127.0.0.1:54321/postgres'},{INBOX_ACTION_DATABASE_URL:'postgres://inbox_action_worker:synthetic@127.0.0.1:54322/other'},{INBOX_ACTION_DATABASE_URL:'postgres://postgres:synthetic@127.0.0.1:54322/postgres'},{NODE_ENV:'production'},{INBOX_ACTION_LOCAL_FIXTURE:'0'}])assert.throws(()=>databaseConfiguration({...env,...patch}));
});
