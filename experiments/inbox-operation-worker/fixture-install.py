#!/usr/bin/env python3
"""Compile/once-install metadata companions in the fixed full canonical fixture."""
...
D=['docker','--host','unix:///Users/jarradhenry/.colima/inbox-redesign-20260913/docker.sock'];N='sandra-inbox-projection-t2-db';DB='sandra_inbox_action_runtime_20260913';MARKER='sandra-inbox-action-runtime-owned-synthetic'
...
(P/'fixture-companion.sql').write_text(compiled)
...
(P/'fixture-companion-manifest.json').write_text(json.dumps(receipt,indent=2)+'\n')
...
need(sql('SELECT count(*) FROM inbox_operations.operations')=='0','Fresh fixture has copied accepted jobs')
...
(P/'fixture-install-evidence.json').write_text(json.dumps(receipt,indent=2)+'\n')
print('Fixed full-schema fixture companion installed; source, captures, role authority and baseline checked')
