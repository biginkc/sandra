...
CREATE FUNCTION inbox_t2_policy.validate_key(ns text,k jsonb) RETURNS void LANGUAGE plpgsql IMMUTABLE SECURITY DEFINER SET search_path='' AS $$
...
BEGIN
 ...
 FOR event IN SELECT value FROM jsonb_array_elements(events) LOOP
  ...
  PERFORM inbox_t2_policy.validate_key(event->>'namespace',event->'key');
 END LOOP;
...
BEGIN
 ...
 FOR requirement IN SELECT value FROM jsonb_array_elements(requirements) LOOP
  ...
  PERFORM inbox_t2_policy.validate_key(requirement->>'namespace',requirement->'key');
 END LOOP;
