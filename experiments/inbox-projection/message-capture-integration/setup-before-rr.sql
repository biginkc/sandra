...
BEGIN
 ...
 INSERT INTO inbox_t2_message_capture.sender_buckets VALUES(p_org,h) ON CONFLICT DO NOTHING;
 PERFORM 1 FROM inbox_t2_message_capture.sender_buckets WHERE org_id=p_org AND raw_hash=h FOR UPDATE;
 SELECT sender_group_id INTO found_id FROM inbox_t2_message_capture.sender_groups
 WHERE org_id=p_org AND raw_hash=h AND raw_sender COLLATE "C"=p_raw COLLATE "C";
