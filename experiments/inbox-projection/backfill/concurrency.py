...
checks=[]
for mode in ['update','delete','org_move','late_claim']:
 o,c,e,m,v,newv,o2=[uid() for _ in range(7)];holder=worker=None
 try:
  sql(f"BEGIN;INSERT INTO organizations(id,name) VALUES('{o}','Backfill concurrency {o}'),('{o2}','Backfill destination {o2}');INSERT INTO contacts(id,org_id,first_name) VALUES('{c}','{o}','Synthetic');INSERT INTO properties(id,org_id,address,state,homeowner_contact_id) VALUES('{e}','{o}','Synthetic {e}','MO','{c}');INSERT INTO messages(id,org_id,conversation_id,property_id,contact_id,channel,direction,status,body,from_address) VALUES('{m}','{o}','{v}','{e}','{c}','sms','inbound','received','Backfill concurrency','+18165550895');SELECT inbox_t2_backfill.start('{o}');COMMIT;")
  ...
  mutation=f"UPDATE messages SET from_address='+18165550896',conversation_id='{newv}' WHERE id='{m}'" if mode=='update' else f"UPDATE messages SET org_id='{o2}',conversation_id='{newv}',from_address='+18165550896' WHERE id='{m}'" if mode=='org_move' else f"DELETE FROM messages WHERE id='{m}'" if mode=='delete' else f"DO $$ BEGIN PERFORM generation FROM inbox_t2_message_capture.dirty WHERE org_id='{o}' AND target_kind='known_conversation' AND target_id='{v}' FOR UPDATE;END $$"
  ...
  if mode=='update':
   ...
   need(result['source_rows']==0,'Source tenant predicate was not rechecked after lock wait')
   need(sql(f"SELECT count(*) FROM inbox_t2_message_capture.route_edges WHERE org_id='{o}' AND message_id='{m}'")=='0','Old organization edge resurrected')
   edge=json.loads(sql(f"SELECT to_jsonb(e) FROM inbox_t2_message_capture.route_edges e WHERE org_id='{o2}' AND message_id='{m}'"))
   need(edge['phone_e164']=='+18165550896' and edge['conversation_id']==newv,'Current destination edge lost')
   need(int(sql(f"SELECT generation FROM inbox_t2_message_capture.dirty WHERE org_id='{o}' AND target_kind='known_conversation' AND target_id='{v}'"))==int(generation)+1,'Departing direct invalidation lost')
   need(int(sql(f"SELECT generation FROM inbox_t2_message_capture.dirty WHERE org_id='{o2}' AND target_kind='known_conversation' AND target_id='{newv}'"))>0,'Destination direct invalidation lost')
   checks.append({'name':'source tenant/conversation/phone move holds row before backfill snapshot; observed wait rechecks org predicate, preserves both direct invalidations and destination edge, and never resurrects departing edge','passed':True})
  elif mode=='delete':
   ...
   checks.append({'name':'old worker blocks on private child after source locks; replaced claim preserves the known dirty counter and replacement checkpoint; replacement progresses','passed':True})
 finally:stop(worker);stop(holder)
evidence={'at':sql('SELECT statement_timestamp()::text'),'checks':checks,'setup_sha256':hashlib.sha256((P/'setup.sql').read_bytes()).hexdigest(),'runner_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'limits':['Real independent sessions and observed lock waits; lease expiry simulated by private fixture update','No universal deadlock freedom, production workload or crash recovery claim','Late-fence assertions cover the known dirty counter and checkpoint, not an absent unknown registry or changed edge','Concurrent collision inspector versus a newer generation is source-reviewed but not runtime-tested here']}
