...
- [x] Integrate history with authenticated head snapshots; retain actual nested custom/generic plans.
- [x] Preserve known and unknown grouping rules in private summary computations.
- [x] Integrate the application boundary codec with private capture-generation metadata.
- [ ] Exercise authenticated history under realistic concurrent write churn.
...
The initial T2 rehearsal merged in PR #552 as
`580ff7286e29e695bfd2db1faf9ea20c6a452e59` after all repository CI checks passed.
The next read-model slice adds the following reviewed components. None activates
an application route, provider call, or production migration.

| Component | Retained evidence | Remaining integration |
| --- | --- | --- |
| [Authenticated detail](authenticated-detail/README.md) | Five grouped authorization checks per version; four actual v2 nested plans. V1 generic deep pagination rejected 90,000 keys; separate v2 pushes the cursor into the index. | Trusted JWT/HTTP context, response-time access recheck, concurrent ingest and production workload. |
| [Known summaries](summary-contract/README.md) | 21 checks, including 28 legacy RPC row comparisons and exact visibility sets. | Complete dirty dependency capture, expiry scheduling, maintained publication and large-history cost. |
| [Unknown summaries](unknown-summary-contract/README.md) | 12 checks using the actual legacy classifier across 1,013 rows. | Persistent typed identities, frozen durable worksets, authorization and command receipts. |
| [Capture boundary](capture-boundary-integration/README.md) | Six database/actual-codec checks, including rotation during a held snapshot. Application codec has 34 unit cases. | Acceptance/mutation fencing, read receipts and live route integration. |

Independent reviews found no blocking issues within these limited scopes.
`read-model-review-manifest.json` records the bytes reviewed after the runs; it
does not retroactively claim execution-time hashes missing from older receipts.
The production signing key is neither configured nor generated by this change.
The application integrity primitive remains unused and does not grant access.
