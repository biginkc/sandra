...
parser=argparse.ArgumentParser();parser.add_argument('--run-owned-fixture',action='store_true');parser.add_argument('--continue-installed',action='store_true');parser.add_argument('--candidate-version',choices=['v1','v2'],default='v1');args=parser.parse_args()
...
def request(tenant=org,conversation=conv,requester=user,role='authenticated',cursor=None):
 ...
 return f"BEGIN READ ONLY;SET LOCAL ROLE {role};SET LOCAL request.jwt.claims={lit(claims)};SELECT inbox_t2_authenticated_detail.{FUNCTION}('{tenant}','{conversation}'{extra});ROLLBACK;"
...
try:
 if args.continue_installed:
  installed=json.loads(sql(f"SELECT jsonb_build_object('body',prosrc,'owner',pg_get_userbyid(proowner),'stable',provolatile='s','definer',prosecdef,'config',proconfig) FROM pg_proc WHERE oid='inbox_t2_authenticated_detail.{FUNCTION}(uuid,uuid,timestamptz,uuid)'::regprocedure;"))
  expected=re.search(r'CREATE FUNCTION.*?AS \$\$(.*?)\$\$;', SETUP.read_text(), re.S).group(1)
