...
for name,signature in [('capture_parent',''),('capture_review',''),('claim','integer,integer'),('batch','uuid,text,uuid,uuid,integer')]:
 body=(P/'setup.sql').read_text().split('CREATE FUNCTION inbox_t2_parent.'+name,1)[1].split('AS $$',1)[1].split('$$;',1)[0]
 need(sql(f"SELECT prosrc FROM pg_proc WHERE oid='inbox_t2_parent.{name}({signature})'::regprocedure").strip()==body.strip(),'Installed source mismatch')
o,c,e,m,v=[str(uuid.uuid4()) for _ in range(5)];holder=worker=None
try:
 sql(f"BEGIN;INSERT INTO organizations(id,name) VALUES('{o}','Parent concurrency {o}');INSERT INTO contacts(id,org_id,first_name) VALUES('{c}','{o}','Synthetic');INSERT INTO properties(id,org_id,address,state,homeowner_contact_id) VALUES('{e}','{o}','Synthetic {e}','MO','{c}');INSERT INTO messages(id,org_id,conversation_id,property_id,contact_id,channel,direction,status,body) VALUES('{m}','{o}','{v}','{e}','{c}','sms','inbound','received','Parent concurrency');COMMIT;")
 def claim():return sql(f"SELECT claim_token FROM inbox_t2_parent.claim(100,300) WHERE org_id='{o}' AND kind='property' AND entity_id='{e}'")
 ...
 app='parent-fence-'+o;worker=subprocess.Popen(CMD,stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
 worker.stdin.write(f"SET statement_timeout='15s';SET application_name='{app}';SELECT inbox_t2_parent.batch('{o}','property','{e}','{token}',2);\n");worker.stdin.close()
 ...
 sql(f"UPDATE inbox_t2_parent.work SET lease_until=statement_timestamp()-interval '1 second',available_at=statement_timestamp()-interval '1 second' WHERE org_id='{o}' AND kind='property' AND entity_id='{e}'")
 ...
 need(sql(f"SELECT cursor IS NULL AND claim_token='{replacement}'::uuid FROM inbox_t2_parent.work WHERE org_id='{o}' AND kind='property' AND entity_id='{e}'")=='t','stale batch changed replacement cursor/token')
 result=json.loads(sql(f"SELECT inbox_t2_parent.batch('{o}','property','{e}','{replacement}',2)"));need(result['result']=='next_stream','replacement failed to make progress')
 evidence={'at':sql('SELECT statement_timestamp()::text'),'checks':[{'name':'real worker blocks after source snapshot; replaced claim fences old cursor and rolls back child enqueue; replacement progresses','passed':True}],'setup_sha256':hashlib.sha256((P/'setup.sql').read_bytes()).hexdigest(),'runner_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'limits':['Lease expiry simulated by fixture-private update; actual concurrent sessions and lock wait verified','No process crash, deadlock freedom, production workload or provider execution claim']}
