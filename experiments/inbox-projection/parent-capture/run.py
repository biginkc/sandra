...
message_base=(uuid.uuid4().int >> 8) << 8
messages=[str(uuid.UUID(int=message_base+i)) for i in range(10,17)];convs=[uid() for _ in messages]
...
late=str(uuid.UUID(int=message_base+1));lateconv=uid();sql(ins(late,lateconv));need(dirty(lateconv)>0,'Behind-cursor insertion lost')
...
for _ in range(20):
 if state('property',prop)['scan_generation'] is None:break
 batch('property',prop)
else:raise RuntimeError('Active scan exceeded bounded drain')
...
for _ in range(20):
 if state('property',prop)['generation']==state('property',prop)['ack']:break
 batch('property',prop)
else:raise RuntimeError('Parent pending work exceeded bounded drain')
...
for _ in range(20):
 if state('contact',contact)['generation']==state('contact',contact)['ack']:break
 batch('contact',contact)
else:raise RuntimeError('Contact work exceeded bounded drain')
