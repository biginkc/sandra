...
- Kanban production canary coverage now creates a canary-owned lead, drags it
  from New Lead to Contacted through the real UI, verifies the persisted status
  change, confirms it survives reload, and cleans up the canary property.
- Lead management production canary coverage now creates a canary-owned lead,
  edits status, motivation, and assignee through the lead detail UI, verifies
  the persisted row, confirms the controls survive reload, and cleans up the
  canary property.
- SMS template production canary coverage now creates, edits, and soft-deletes
  a canary-owned template through the real UI, verifies persisted and visible
  table state, and hard-cleans the canary template row afterward.
- Outbound SMS production canary coverage now requires an allowlisted
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Auth | Login as the dedicated canary user, persist session, reload, and confirm protected app access. | Production Supabase Auth, production app. | Auth session only. | Test-project E2E auth setup exists; production canary authenticates the dedicated user and persists storage state before app specs run. | Add scheduled workflow wiring once production canary secrets are installed. | Dedicated low-privilege canary user; fail if URL is not production. | P0 | Pre-release and post-deploy |
| Auth and membership | Confirm canary user only sees tenant-scoped data and cannot access another tenant's records. | Production Supabase Auth, RLS, production app. | Canary tenant rows. | Test-project membership/RLS coverage exists; production canary now seeds visible and hidden canary-org rows and verifies `/properties` only renders the authenticated user's org row. | Add scheduled workflow wiring once production canary secrets are installed. | Canary tenant only; no cross-tenant destructive writes. | P0 | Pre-release |
| Dashboard shell | Load authenticated dashboard, navigate primary sections, and verify protected routes do not leak unauthenticated content. | Production app, production auth. | None or canary audit row if added later. | Local/test E2E shell coverage exists; production canary now verifies protected `/properties`, `/leads`, and `/messages` shell navigation after login. | Add scheduled workflow wiring once production canary secrets are installed. | Read-only after login. | P0 | Post-deploy |
...
| Import updates | Re-import matching canary rows and verify update mode changes existing records instead of duplicating them. | Production app, production DB, import update logic. | Updated canary property/prospect/contact rows, import job rows. | Test-project update wizard spec exists; production canary now previews matched/unmatched rows, applies status updates, verifies update job counters and persisted statuses, and confirms canary rows still render in `/properties`. | Add scheduled workflow wiring once production canary secrets are installed. | Use unique canary addresses; cleanup by run id. | P0 | Pre-release |
...
| Lead management | Open a canary lead, edit details, assign owner/status, and verify persistence after reload. | Production app, production DB. | Lead updates, assignment/status rows if applicable. | Test-project cockpit/lead detail specs exist; production canary now edits status, motivation, and assignee on a canary lead detail page and verifies persisted state after reload. | Add scheduled workflow wiring once production canary secrets are installed. | Dedicated canary lead; restore/delete after run. | P1 | Pre-release |
| Kanban | Drag a canary lead between pipeline columns and verify persisted status. | Production app, production DB. | Lead status/stage updates. | Test-project Kanban drag spec exists; production canary now creates a canary lead, drags it from New Lead to Contacted, verifies persisted status, and confirms reload visibility. | Add scheduled workflow wiring once production canary secrets are installed. | Canary lead only; cleanup or restore original status. | P1 | Pre-release |
| Messaging outbound | Send an SMS from the UI to an owned allowlisted number and verify provider send plus UI thread update. | Production app, production DB, real SMS provider. | Message row, provider delivery/log row, contact activity. | Script-level production SMS canaries exist; production Playwright canary now sends from the lead-detail UI to an allowlisted recipient and verifies persisted `sent` state plus thread rendering. | Add scheduled workflow wiring once provider canary secrets are installed. | Allowlisted recipient; message body includes canary run id. | P0 | Pre-release and scheduled |
...
| Templates | Create, edit, use, and delete an SMS template in a real send-capable flow. | Production app, production DB, SMS provider if used in send step. | Template row, message row if send is included. | Production canary now creates, edits, soft-deletes, verifies visible table state, and hard-cleans a canary-owned SMS template. | Add send-capable template usage inside the outbound messaging canary. | Canary template name; delete after run. | P1 | Pre-release |
...
| Dialer/Jitter handoff | Create a Sandra dialer batch, verify Jitter consumes it, and verify Sandra receives call activity/writeback. | Sandra production app/DB, Jitter production integration, real provider path where applicable. | Dialer batch/items, call activity, recording/transcript metadata if produced. | Dialer batch create E2E exists in test project; production canary now creates a canary batch through the UI, registers a canary Jitter writeback consumer, verifies signed batch fetch/claim/writeback endpoints, and confirms the call appears on the lead. | Add true Jitter app/browser coverage once the deployed Jitter consumer is available. | Canary batch/list only; cleanup batch/items/writeback rows. | P0 | Pre-release |
