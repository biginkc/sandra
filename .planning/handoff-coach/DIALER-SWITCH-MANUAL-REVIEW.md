...
## CI fixture timing follow-up

Final-head hosted verify passed its full unit/component/database gate, then the synthetic suite failed one of 69 cases because the audio fixture's single transcript broadcast used a fixed 100ms timer and arrived before subscription. Replaced the timer with an observable subscriber-readiness check and one explicit seed before audio stimulation. The existing transcript and transport assertions are unchanged; missing readiness still fails within the test deadline. No production code changed. All 39 repeated audio cases pass. The manual tests reviewer and Claude Fable 5.1 independently approved this delta. Fable noted a pre-existing stub limitation for multiple listeners per channel; the current coach registers one listener, so this is not a defect in the change.
