...
- Exact-head correction review at `0d83ea8` rejected two under-specified assertions: cross-route tests checked only the queued body, not its `Queued · in Outbox` label on the destination, and the DNC test did not exclude the legacy actionable control. Both findings were accepted and fixed by scoping the delivery label to the matching destination bubble and asserting the legacy `dispo-dnc` selector is absent. The two design checks and two modified queue-parity paths pass locally; an unchanged realtime webhook case collided with concurrent shared-test-database cleanup and is not counted as correction proof.
