...
### Round 25 — unified timeline UI review

- Surface: `lead_events` is the fourth source in the existing lead-detail activity feed. The page fetches the newest 200 rows; the client interleaves them with messages, notes, and calls; compact centered pills render only bounded plain-English fields. The existing SMS bubble component and its grouping/metadata behavior remain unchanged.
- Provider alignment: the implementation follows Supabase's documented `postgres_changes` INSERT subscription with a server-side `property_id` filter, authenticates Realtime with the current session token, and removes each channel during cleanup. A post-subscribe bounded reconciliation closes the server-render-to-subscription gap.
- Race corrections from adversarial review: same-lead server refreshes trigger a fresh reconciliation even while the last successful rows remain visible; each lifecycle uses a unique topic to avoid reusing a singleton channel that is still leaving; retry/server snapshots merge by event ID with concurrent INSERTs and retain the newest 200; route-stale property rows are filtered out.
- Formatter/privacy corrections: mixed-offset sub-millisecond timestamps sort by instant; current/former/unknown teammates never expose UUIDs or email domains; supported writer shapes render directly; malformed or unknown payloads fall back to `recorded activity` without raw payload fields. Both single and counted sequence resumes are covered.
- Failure behavior: a failed Activity source remains isolated from messages, notes, and calls; a successful client reconciliation clears the stale server error; a later same-lead failure retains previously recovered activity while the error is shown. Manual retry cannot erase a concurrent Realtime event.
- Build boundary: the production build exposed an existing client component importing a now server-only CASS module. One 11-line client-safe predicate leaf plus direct pricing import fixes that graph boundary; the server module re-exports the predicate for compatibility. No CASS behavior changed.
- Independent final review: correctness, boundary, and test lanes all returned no findings. Fable's exact final diff verdict is `APPROVE`, high confidence, zero blockers, and `ANTIBLOAT: PASS`.
- Verification: focused current-diff suites pass 82/82 plus the final 10/10 UI/realtime checks; typecheck and touched-file lint pass; all 240 unit files pass (2507/2507). The full RTL run had one known unrelated `prospects-table` menu timing flake and passed that exact file immediately in isolation (39/39). Production build passes with only the inherited handled `/templates` dynamic-render log.
- Repository lint baseline: `npm run lint -- --quiet` reports the same 364 errors on this branch and a clean detached `origin/main`; normalized outputs are byte-for-byte identical. No unrelated lint cleanup is included.
- Scope verdict: proportional for at most 100 users. No pagination framework, global store, subscription manager, schema-validation layer, queue, cache, service, or optional timeline controls were added.
- Next bounded step: commit and push this approved slice, then create the PR with the exact dependency and three required out-of-scope follow-ups before exact-head CI and browser acceptance.
