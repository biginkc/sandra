...
- 2026-08-24: final-head activity review found bounded-window edge cases hidden by the first happy-path E2E. Message reads now exclude sibling-property rows before the 200-row limit; calls use two disjoint bounded windows merged by canonical `started_at ?? created_at` time to the exact logical top 20; realtime message updates remove rows that move outside the lead; retried/realtime note authors are resolved from an active, unexpired, organization-scoped membership allowlist. The first full-auth-map proposal was rejected because Auth spans every organization; cross-org, inactive, expired, and deletion-prepared users are now filtered before client serialization.
- 2026-08-24: the working-state bar now follows the ZIP order Status, Motivation, Assignee. Focused E2E seeds interleaved message, note, and call rows and asserts the exact six-event source order before the composer.
- 2026-08-24: two callback-based preview bootstrap attempts fell back to fragment-token login errors. Each exposed one-time browser session was immediately revoked and its temporary screenshot removed. The verified non-email acceptance bootstrap exchanges an admin-generated token hash directly, serializes the resulting SSR cookies only into the isolated visible Chrome acceptance context, and revokes that session when acceptance ends.
- 2026-08-24: post-hardening verification passes 228 unit files / 2,392 tests, 87 RTL files / 802 tests, typecheck, production build, and Chrome-channel focused E2E setup plus both design-fidelity scenarios (3/3). The build retains only the repository's pre-existing `/templates` dynamic-cookie diagnostics and exits successfully.
- 2026-08-24: final self-audit found that a single Auth page could degrade valid teammate authorship in projects with more than 1,000 auth users. Identity resolution now advances locally through bounded 200-user pages (maximum 25), filters every page through the active organization allowlist, stops on full resolution or a short page, and fails closed on any page error. Focused contracts, typecheck, ESLint, and diff checks pass.
...
- 2026-08-24: final parity review then exercised hidden branches omitted from the normal lead screenshots. Flat-media `PageHeader` actions now wrap inside a mobile-safe `min-w-0` container and inherit 36px button targets; the expanded custom-tag input is also 36px tall and width-bounded. Focused flat-render, tag-height contract, typecheck, ESLint, and diff checks pass.
