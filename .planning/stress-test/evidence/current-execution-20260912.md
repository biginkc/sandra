...
| J08-03 fresh implicit handoff | `20000000-0000-4000-8000-000000001011` | PASS, browser | Browser submitted Needs nurture on an untouched version-0 lead; reload removed the row from the rep queue and SQL showed the archived sentinel, ended rep episode, and owner episode. |
...
- `J05-01` archive cancellation reset: on `20000000-0000-4000-8000-000000001042`, the browser ticked then unticked the confirmation, clicked Cancel, reopened it, and clicked Archive lead without a fresh tick; validation kept the dialog open. No archive mutation occurred.
...
- `J02-01` explicit no motivation: prepared `20000000-0000-4000-8000-000000001032` with known Warm temperature, selected No motivation provided with Keep temperature unchanged, and reloaded into Needs offer / Interested. SQL before/after remained Warm with `motivation_kind=no_motivation`, and task count stayed 0.
- `J02-02` motivation mode switch and draft reset: on `...001030`, switching specified to No motivation to specified cleared the draft and Save showed the required error; on second lead `...001031`, dirty Cancel then reopen produced an empty textarea. No cross-lead draft leaked.
...
- `J10-00` stale tab: rep browser held a valid attempt form for `...001024` while owner browser handed it off; Save returned `This lead changed`, and reload showed zero old-rep rows. `J10-01` then used assigned fixture `...001034`: owner logged a Reached attempt while the rep form was open, rep Save returned `This lead changed`, full reload cleared the stale dialog, the row reopened, and detail showed exactly two attempts with no duplicate retry.
...
- `J16-02` direct-offer motivation recovery: on `...001033`, opening Log offer and clicking blank Save surfaced the required motivation and offer-field errors; selecting No motivation removed the motivation error. Valid sent/follow-up timestamps could not be committed through the native datetime controls, so persistence remains BLOCKED; rerun with the reliable datetime harness and verify the offer plus no side effects.
...
| J02-00, J02-01, J02-02 | Contacted → Needs offer / Interested for specified and explicit no-motivation paths; the Warm before/after value stayed unchanged for J02-01, and the mode-switch/draft branch left no write. | No task or appointment appeared; reload, SQL, and dialog validation matched. |
| J05-01, J08-01, J08-02, J08-03 | Archive reset validation left the contract row unchanged; recipient race/blank reason left handoff unchanged; fresh version-0 handoff moved the row to archived sentinel and owner episode. | No partial archive/handoff write; settings restored and no external provider effect. |
...
| J10-00, J10-01 | Rep row/form open → stale Save error; J10-01 then reloaded and reopened the still-assigned row with exactly two attempts. | No stale retry was written; owner-side attempt was the only new event and no duplicate appeared. |
...
- Missing recipient or unsupported lifecycle re-entry: `J04-01`, `J04-02`, `J04-03`, `J05-00`, `J05-02`, `J08-00`, `J16-00`, `J16-03`, and `J17-02`. No partial state was inferred from an unexercised browser commit.
...
- Fault injection or auth-revocation capability: `J10-02`, `J23-00`, `J23-01`, `J23-02`, `J24-00`, `J24-01`, and `J24-02`. No network, session, or permission failure was fabricated as a product result.
...
- All 84 CSV cases are terminal: 27 PASS and 57 BLOCKED. BLOCKED rows identify the unavailable clock, fault-injection, identity, side-effect, native-control capability, or incomplete branch coverage; they are not product failures. Each CSV row includes a concrete setup check, next action, and retained-fixture/writer status.
