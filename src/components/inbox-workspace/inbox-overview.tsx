...
export function InboxOverview({ identity }: { identity: InboxQueryIdentity & { expiresAt: number } }) {
  ...
  // A canonical 401/403 on counts is access loss (session/membership revoked between
  // /context succeeding and /counts responding), not a benign retryable count failure —
  // mirrors the workspace's accessLost, distinct from a transient "Counts unavailable".
  const [countsAccessLost, setCountsAccessLost] = useState(false);
  const [attempt, setAttempt] = useState(0);
  const denied = access === "denied" || countsAccessLost;
  useEffect(() => {
    ...
    void cache.read<InboxCounts>("counts", "overview", async signal => {
      ...
      if (response.status === 401 || response.status === 403) { if (active) setCountsAccessLost(true); throw Error("Access lost"); }
  ...
  return <main className="min-h-dvh bg-background p-6 md:p-10"><a href="/messages">← Back to Messages</a>
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{filters.map(([view, label]) => <a key={view} href={`/inbox?view=${view}`} className="rounded-xl border p-5"><h3>{label}</h3><strong className="mt-3 block text-3xl">{authorized && counts && !denied ? counts.counts[view] : "—"}</strong></a>)}</div>
    {(denied || access === "unavailable") && <button className="mt-4 underline" onClick={() => window.location.reload()}>Reload overview</button>}
    {error && authorized && !denied && <button className="mt-4 underline" onClick={() => { setCounts(null); setError(false); setAttempt(value => value + 1); }}>Retry counts</button>}
    <p role="status" className="mt-4 text-sm text-muted-foreground">{countsAccessLost ? "Your access has changed. Reload to continue." : authorized && counts ? `Counted at ${new Date(counts.asOf).toLocaleTimeString()}` : !authorized ? "Verifying access. Counts are hidden until access is confirmed." : error ? "Counts unavailable. You can still open the workspace." : "Loading counts. You can open the workspace now."}</p>
  </main>;
}
