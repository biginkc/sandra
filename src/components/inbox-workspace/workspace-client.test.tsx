...
const state = vi.hoisted(() => ({ callbacks: null as null | { onChange: (value: unknown) => void; onAccessBoundary: () => void; onInvalidated: (ids: readonly string[]) => void }, replacements: [] as unknown[], deny: false }));
...
beforeEach(() => {
  ...
  calls = []; state.replacements = []; state.deny = false;
  vi.stubGlobal("fetch", vi.fn(async (url: string) => {
    ...
    if (url.includes("read-acknowledgments")) return Response.json({ boundaryId: "boundary", batch: 0, changed: 1, completed: true });
