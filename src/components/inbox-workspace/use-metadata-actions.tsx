...
import type { PreparedInboxAction, PrepareInboxActionRequest, AcceptedInboxAction, InboxOperationStatus, InboxMetadataOutcome, InboxAssigneeChoice, InboxMetadataStep } from "@/lib/inbox/action-api-contract";
...
type WorkspaceMetadataStep = InboxMetadataStep | { type: "promote" | "dismiss_unknown" | "restore_unknown" };
type WorkspacePrepareInboxActionRequest = Omit<PrepareInboxActionRequest, "definition"> & {
    definition: { version: 1; steps: readonly WorkspaceMetadataStep[] };
};
type Draft = {
    request: WorkspacePrepareInboxActionRequest;
...
export function useInboxMetadataActions(options: Options) {
    ...
    function makeDraft(ids: readonly WorkspaceId[], steps: readonly WorkspaceMetadataStep[], assigneeLabel?: string): Draft {
    ...
    function prepare(actionId: string, ids: readonly WorkspaceId[]) {
        ...
        const directStep = actionId === "promote" || actionId === "dismiss_unknown" || actionId === "restore_unknown" ? { type: actionId } as const : undefined;
        if (!outcome && !directStep && actionId !== "unassign")
            return;
        void prepareDraft(makeDraft(ids, directStep ? [directStep] : outcome ? [{ type: "outcome", value: outcome }] : [{ type: "assign", userId: null }]));
    }
    async function accept() {
        ...
        if (!recovery.remember(pair)) {
            setDraft({ ...next, stage: "prepared", acceptAttempted: true, error: "This tab already has 50 unresolved actions. Check their receipts before starting another." });
            return;
        }
