...
import { createCloserServiceClient } from "./service-client";

type CloserServiceClient = SupabaseClient<Database>;

export type CloserAuthOk = {
  ok: true;
  consumerId: string;
  orgId: string;
  serviceClient: CloserServiceClient;
  rawBody: string;
};

export type CloserAuthResult =
  | CloserAuthOk
...
function unauthorized(): CloserAuthResult {
  return {
    ok: false,
...
export async function authenticateCloserPractice(
  request: Request,
): Promise<CloserAuthResult> {
  ...
  const serviceClient = createCloserServiceClient();
  ...
  const { data: consumer, error } = await (serviceClient as any)
    ...
    .eq("consumer_type", "closer_practice")
    .eq("enabled", true)
    .is("revoked_at", null)
    .maybeSingle();
  ...
  const { error: lastUsedError } = await (serviceClient as any)
    ...
    .eq("id", consumer.id);
  if (lastUsedError) {
    console.warn("[authenticateCloserPractice] last_used_at update failed", {
