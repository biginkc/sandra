...
const mocks = vi.hoisted(() => ({ create: vi.fn(), acknowledge: vi.fn(), getUser: vi.fn(), memberships: vi.fn() }));
...
describe("read-acknowledgments deployment boundary", () => {
  ...
  it("returns 404 and never calls the acknowledge RPC for an acquisitions-only member", async () => {
