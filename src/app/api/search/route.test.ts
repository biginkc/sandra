...
describe("global search route", () => {
  ...
  it("returns accessible lead hits to restricted members and omits conversation-only contacts and threads", async () => {
    ...
    rpc.mockResolvedValue({ data: [
      { entity_type: "thread", entity_id: "m1", conversation_id: "c1", property_id: "p1", title: "Property thread", subtitle: "Sensitive inbound message preview", matched_field: "phone" },
      { entity_type: "thread", entity_id: "m2", conversation_id: "c2", property_id: null, title: "Orphan thread", subtitle: "Sensitive orphan message preview", matched_field: "phone" },
    ...
    expect(results.map((row: { href: string }) => row.href)).toEqual(["/leads/p2", "/leads/p3"]);
    expect(results).not.toEqual(expect.arrayContaining([
      expect.objectContaining({ subtitle: "Sensitive inbound message preview" }),
      expect.objectContaining({ subtitle: "Sensitive orphan message preview" }),
    ]));
  });
