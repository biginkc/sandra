...
export default async function SavedCalculationPage({params}:{params:Promise<{id:string;calculationId:string}>}) {
  ...
  return <Page className="bg-stone-100 text-stone-900">
    <PageHeader breadcrumb={[{label:'Workspace'},{label:lead.address,href:`/leads/${id}#calculations`},{label:`Revision v${saved.version}`}]} title={`Offer calculation · v${saved.version}`} description={`${lead.address} · ${lead.seller}`} actions={canEdit?<Link className="rounded-full bg-stone-900 px-5 py-3 text-sm font-semibold text-white" href={`/calculators?calculationId=${saved.id}`}>Create a new version</Link>:null}/>
