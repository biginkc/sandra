...
export function createLeadEsignRepository(): EsignActionRepository {
  return {
    ...
    claimReminder: async ({ orgId, requestId, signerId }) => {
      ...
      if (error) throw error;
      const row = data?.[0];
      if (!row || row.outcome === "ineligible") return { outcome: "ineligible" };
      if (row.outcome === "cooldown") return { outcome: "cooldown" };
      if (row.outcome === "in_progress") return { outcome: "in_progress" };
      if (row.outcome !== "claimed") throw new Error("Unexpected reminder claim outcome.");
    ...
    finalizeReminder: async ({ orgId, requestId, signerId, claimToken }) => {
      const { data, error } = await createAdminClient().rpc(
        ...
        { p_org_id: orgId, p_request_id: requestId, p_signer_id: signerId, p_claim_token: claimToken },
      );
      if (error) throw error;
    ...
    claimVoid: async ({ orgId, requestId }) => {
      const claimToken = randomUUID();
      const { data, error } = await createAdminClient().rpc("claim_esign_request_void", {
        p_org_id: orgId,
        p_request_id: requestId,
        p_claim_token: claimToken,
      });
      ...
      if (!row || row.outcome === "ineligible") return { outcome: "ineligible" };
      if (row.outcome === "in_progress") return { outcome: "in_progress" };
      if (row.outcome !== "claimed") throw new Error("Unexpected void claim outcome.");
      const request = await loadRequest(orgId, requestId);
      if (!request) {
    ...
    finalizeVoid: async ({ orgId, requestId, claimToken }) => {
      const { data, error } = await createAdminClient().rpc("finalize_esign_request_void", {
        p_org_id: orgId,
        p_request_id: requestId,
        p_claim_token: claimToken,
      });
...
async function releaseReminderRpc(orgId: string, requestId: string, signerId: string, claimToken: string) {
  const { data, error } = await createAdminClient().rpc("release_esign_signer_reminder", {
    p_org_id: orgId, p_request_id: requestId, p_signer_id: signerId, p_claim_token: claimToken,
  });
  if (error) throw error;
...
async function releaseVoidRpc(orgId: string, requestId: string, claimToken: string) {
  const { data, error } = await createAdminClient().rpc("release_esign_request_void", {
    p_org_id: orgId, p_request_id: requestId, p_claim_token: claimToken,
  });
  if (error) throw error;
  return data === "released" ? "released" as const : "lease_lost" as const;
}

async function loadLeadSendContext({ actor, propertyId }: { actor: EsignActor; propertyId: string }): Promise<LeadSendContext | null> {
  ...
  const [contactResult, integrationResult, templatesResult] = await Promise.all([
    property.homeowner_contact_id
      ? admin.from("contacts").select("first_name,last_name,entity_name,contact_type,email").eq("id", property.homeowner_contact_id).eq("org_id", actor.orgId).maybeSingle()
      : Promise.resolve({ data: null, error: null }),
    admin.from("org_esign_integrations").select("sending_enabled,test_mode").eq("org_id", actor.orgId).maybeSingle(),
    admin.from("available_esign_templates").select("id,name,document_type,sign_template_id,seller_role,signer_roles,merge_field_names").eq("org_id", actor.orgId).order("updated_at", { ascending: false }),
  ]);
  if (contactResult.error) throw contactResult.error;
  if (integrationResult.error) throw integrationResult.error;
  if (templatesResult.error) throw templatesResult.error;
  const contact = contactResult.data;
  const sellerName = contact?.contact_type === "entity"
    ? contact.entity_name ?? ""
...
function toTemplateOption(row: { id: string | null; name: string | null; document_type: string | null; sign_template_id: string | null; seller_role: string | null; signer_roles: Json | null; merge_field_names: string[] | null }): TemplateOption[] {
  const roles = parseRoles(row.signer_roles);
  if (!row.id || !row.name || !row.document_type || !row.sign_template_id || !row.seller_role || !roles) return [];
  if (!sameMergeFields(row.merge_field_names)) return [];
  return [{ id: row.id, name: row.name, documentType: row.document_type, providerTemplateId: row.sign_template_id, sellerRoleName: row.seller_role, signerRoles: roles, mergeFieldNames: ESIGN_TEMPLATE_MERGE_FIELDS }];
}
...
function sameMergeFields(value: string[] | null): boolean {
  return Boolean(value && value.length === ESIGN_TEMPLATE_MERGE_FIELDS.length && [...value].sort().every((field, index) => field === [...ESIGN_TEMPLATE_MERGE_FIELDS].sort()[index]));
}

async function claimSend(input: Parameters<EsignActionRepository["claimSend"]>[0]): Promise<SendClaim> {
  const { data, error } = await createAdminClient().rpc("create_esign_request", {
...
async function loadRequest(orgId: string, requestId: string): Promise<EsignRequestRecord | null> {
  const admin = createAdminClient();
  const { data: row, error } = await admin.from("esign_requests")
    .select("id,org_id,property_id,template_id,signer_snapshot,merge_value_snapshot,send_intent_id,payload_hash,retry_of_request_id,status,delivery_state,sign_request_id,details_url,void_requested_at,signed_pdf_path,error_message,sent_at,test_mode")
    .eq("org_id", orgId).eq("id", requestId).maybeSingle();
  if (error) throw error;
  if (!row) return null;
  const [{ data: templateRow, error: templateError }, { data: signerRows, error: signerError }, { data: fileRow, error: fileError }] = await Promise.all([
    admin.from("esign_templates").select("id,name,document_type,sign_template_id,seller_role,signer_roles,merge_field_names").eq("org_id", orgId).eq("id", row.template_id).maybeSingle(),
    admin.from("esign_request_signers").select("id,role_name,signer_order,signer_name,signer_email,status,last_reminded_at").eq("org_id", orgId).eq("request_id", requestId).order("signer_order"),
    admin.from("lead_files").select("id").eq("org_id", orgId).eq("source_request_id", requestId).eq("source", "esign_signed_pdf").maybeSingle(),
  ]);
  ...
  return {
    id: row.id, orgId: row.org_id, propertyId: row.property_id, template,
    signers: (signerRows ?? []).map((signer) => ({ id: signer.id, role: signer.role_name, order: signer.signer_order, name: signer.signer_name, emailAddress: signer.signer_email, status: signer.status, lastRemindedAt: signer.last_reminded_at })) as EsignRequestRecord["signers"],
    mergeValues: merge, sendIntentId: row.send_intent_id, payloadHash: row.payload_hash,
    retryOfRequestId: row.retry_of_request_id, status: row.status, deliveryState: row.delivery_state,
    providerRequestId: row.sign_request_id, detailsUrl: row.details_url,
    voidRequestedAt: row.void_requested_at, signedPdfFileId: fileRow?.id ?? null,
  };
}
...
export async function providerForOrg(orgId: string): Promise<EsignActionProvider | null> {
  ...
  return {
    sendWithTemplate: async ({ localRequestId, providerTemplateId, signers, mergeValues, signal }) => {
      try {
        const output = await provider.sendWithTemplate({ localRequestId, templateId: providerTemplateId, signers: signers.map(({ role, name, emailAddress }) => ({ role, name, emailAddress })), mergeValues, signal });
        if (!output.detailsUrl) return { outcome: "ambiguous" };
        return { outcome: "sent", providerRequestId: output.signatureRequestId, detailsUrl: output.detailsUrl, signatures: output.signatures };
      } catch (error) {
        return classifyProviderFailure(error);
      }
    },
    remind: async ({ providerRequestId, signerName, signerEmailAddress, signal }) => {
      try { await provider.remind(providerRequestId, { name: signerName, emailAddress: signerEmailAddress }, signal); return "sent"; }
      catch { return "failed"; }
    },
    cancel: async ({ providerRequestId, signal }) => {
      try { await provider.cancel(providerRequestId, signal); return "accepted"; }
      catch { return "failed"; }
    },
  };
}

function classifyProviderFailure(error: unknown): { outcome: "ambiguous" | "definitive_failure" } {
  if (error instanceof ProviderError) {
    const status = typeof error.details?.statusCode === "number" ? error.details.statusCode : 0;
    if (error.details?.retryable === true || status === 429 || status >= 500) return { outcome: "ambiguous" };
    if (status >= 400 && status < 500) return { outcome: "definitive_failure" };
  }
...
export function createBoundLeadEsignCore() {
  ...
  return createLeadEsignActionCore(createDefaultLeadEsignActionDependencies({
    authenticate: authenticateLeadEsignActor,
    repository,
    providerForOrg,
    files: {
      authorizeSignedFile: async ({ actor, fileId, requestId }) => {
        const { data: file, error } = await createAdminClient().from("lead_files")
          .select("storage_bucket,storage_path,source_request_id")
          .eq("org_id", actor.orgId).eq("id", fileId).eq("source_request_id", requestId).maybeSingle();
        if (error || !file) return null;
        const { data, error: signError } = await createAdminClient().storage.from(file.storage_bucket).createSignedUrl(file.storage_path, SIGNED_URL_SECONDS);
        if (signError || !data?.signedUrl) return null;
        return { url: data.signedUrl, expiresAt: new Date(Date.now() + SIGNED_URL_SECONDS * 1_000) };
      },
    },
  }));
}
...
export async function loadLeadEsignPageModel(propertyId: string): Promise<LeadEsignPageModel> {
  const actor = await authenticateLeadEsignActor();
  if (!actor) return { blockers: ["provider_disconnected"], contracts: [], files: [], contractsError: "Sign in with active organization access.", filesError: "Sign in with active organization access." };
  const repository = createLeadEsignRepository();
  const context = await repository.loadLeadSendContext({ actor, propertyId });
  const blockers: SendBlockerCode[] = !context ? ["provider_disconnected"] : [
    ...
    ...(!context.sellerEmailAddress?.trim() ? ["owner_email_missing" as const] : []),
  ];
  const admin = createAdminClient();
  const [requestsResult, filesResult] = await Promise.all([
    admin.from("esign_requests").select("id").eq("org_id", actor.orgId).eq("property_id", propertyId).order("created_at", { ascending: false }).limit(100),
    admin.from("lead_files").select("id,file_name,created_at,size_bytes").eq("org_id", actor.orgId).eq("property_id", propertyId).eq("source", "esign_signed_pdf").order("created_at", { ascending: false }).limit(100),
  ]);
  ...
  return {
    blockers, contracts,
    files: (filesResult.data ?? []).map((file) => ({ id: file.id, displayName: file.file_name, kind: "signed_contract", createdAt: file.created_at, sizeBytes: file.size_bytes })),
    contractsError: requestsResult.error ? "Contract history is unavailable." : null,
