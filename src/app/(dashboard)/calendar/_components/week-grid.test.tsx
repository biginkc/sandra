...
describe("<WeekGrid />", () => {
  ...
  it("places an appointment in its zone-local day column with a time label that reflects the PASSED timezone, not the runtime default", () => {
    ...
    const conversion = wallTimeToUtc({ date: wednesday.date, time: "14:00", timeZone: LA });
    if (!conversion.ok) throw new Error("fixture conversion failed");
    const dueAt = conversion.utc.toISOString();
    const endAt = new Date(conversion.utc.getTime() + 30 * 60 * 1000).toISOString();
    const appt = makeAppt({ id: "appt-1", due_at: dueAt, end_at: endAt, title: "Walkthrough" });
    ...
    const wedColumn = screen.getByTestId(`calendar-day-column-${wednesday.date}`);
    expect(wedColumn).toHaveTextContent(/2:00.*2:30 PM/);
    expect(screen.getByTestId("calendar-appointment-appt-1")).toBeInTheDocument();
  ...
  it("accents today's column", () => {
    ...
    expect(screen.getByTestId(`calendar-day-column-${days[0].date}`)).not.toHaveAttribute(
      "data-today",
    );
  });
  ...
  it("shows the assignee email for owner role and hides it for member role", () => {
    ...
    const appt = makeAppt({
      ...
      end_at: new Date(new Date(days[0].startUtc).getTime() + 30 * 60 * 1000).toISOString(),
      assignee_id: "rep-1",
    });
  ...
  it("click-through hrefs follow linkage: property -> /leads, contact-only -> /messages?thread, personal block -> unlinked", () => {
    ...
    const appointments = [
      makeAppt({ id: "prop-appt", due_at: start, end_at: end, property_id: "prop-1", address: "123 Main St" }),
      makeAppt({ id: "contact-appt", due_at: start, end_at: end, contact_id: "contact-1", contact_name: "Jane Owner" }),
    ...
    expect(container.querySelector("a[href='/messages?thread=contact-1']")).not.toBeNull();
    expect(screen.getByTestId("calendar-appointment-unlinked-personal-appt")).toBeInTheDocument();
  ...
  it("shows an outcome chip for a completed appointment, and the outcome row ONLY for a past-due open one", () => {
    ...
    expect(screen.queryByTestId("stub-outcome-row-completed")).not.toBeInTheDocument();

    expect(screen.getByTestId("calendar-outcome-chip-completed")).toHaveTextContent("Held");
    expect(screen.queryByTestId("calendar-outcome-chip-past-due")).not.toBeInTheDocument();
    expect(screen.queryByTestId("calendar-outcome-chip-future")).not.toBeInTheDocument();
  });
});
