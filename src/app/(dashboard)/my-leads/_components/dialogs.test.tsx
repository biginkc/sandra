...
it("preserves an initial DialPad call reference and requires a rep outcome", async () => {
  const user = userEvent.setup()
  const onSubmit = vi.fn(async () => ({ ok: true as const }))
  render(<AcquisitionAttemptDialog {...baseProps} onSubmit={onSubmit} initialCallActivityId="dp-call" initialCallSource="dialpad" />)
  expect(screen.getByLabelText("Source")).toHaveValue("dialpad")
  expect(screen.getByLabelText("DialPad call")).toHaveValue("dp-call")
  fireEvent.change(screen.getByLabelText("When did the outreach occur?"), { target: { value: "2026-09-12T10:00" } })
  await user.click(screen.getByRole("button", { name: "Save attempt" }))
  expect(onSubmit).not.toHaveBeenCalled()
  await user.selectOptions(screen.getByLabelText("External outcome"), "reached")
  await user.click(screen.getByRole("button", { name: "Save attempt" }))
  expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ source: "dialpad", callActivityId: "dp-call", outcome: "reached" }))
})
it("filters references by provider and clears evidence when switching to manual DialPad", async () => {
  const user = userEvent.setup()
  const onSubmit = vi.fn(async () => ({ ok: true as const }))
  render(<AcquisitionAttemptDialog {...baseProps} onSubmit={onSubmit} callReferenceOptions={[{id:"dp",label:"DialPad 10 AM",source:"dialpad"},{id:"sj",label:"Sandra 11 AM",source:"sandra"}]} />)
  await user.selectOptions(screen.getByLabelText("DialPad call"), "dp")
  expect(screen.queryByRole("option", {name:"Sandra 11 AM"})).not.toBeInTheDocument()
  await user.selectOptions(screen.getByLabelText("Source"), "sandra")
  expect(screen.getByLabelText("Sandra call")).toHaveValue("sj")
  await user.selectOptions(screen.getByLabelText("Source"), "dialpad")
  expect(screen.getByLabelText("DialPad call")).toHaveValue("")
  await user.selectOptions(screen.getByLabelText("External outcome"), "no_answer")
  fireEvent.change(screen.getByLabelText("When did the outreach occur?"), { target: { value: "2026-09-12T10:00" } })
  await user.click(screen.getByRole("button", { name: "Save attempt" }))
  expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({source:"dialpad",callActivityId:null}))
})

it("resolves a delayed DialPad reference without resetting edited outcome or note", async () => {
  const user = userEvent.setup()
  const onSubmit = vi.fn(async () => ({ ok: true as const }))
  const view = render(<AcquisitionAttemptDialog {...baseProps} onSubmit={onSubmit} initialCallActivityId="dp" callReferencesLoading />)
  await user.selectOptions(screen.getByLabelText("External outcome"), "reached")
  await user.type(screen.getByLabelText("Note (optional)"), "Keep this note")
  fireEvent.change(screen.getByLabelText("When did the outreach occur?"), { target: { value: "2026-09-12T10:00" } })
  await user.click(screen.getByRole("button", { name: "Save attempt" }))
  expect(onSubmit).not.toHaveBeenCalled()
  view.rerender(<AcquisitionAttemptDialog {...baseProps} onSubmit={onSubmit} initialCallActivityId="dp" callReferenceOptions={[{id:"dp",source:"dialpad",label:"10 AM"}]} />)
  expect(screen.getByLabelText("Source")).toHaveValue("dialpad")
  expect(screen.getByLabelText("DialPad call")).toHaveValue("dp")
  await user.click(screen.getByRole("button", { name: "Save attempt" }))
  expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({source:"dialpad",callActivityId:"dp",outcome:"reached",note:"Keep this note"}))
})
