...
describe("conversation selection", () => {
  it("coalesces terminal events without clearing detail or acknowledging it again", async () => {
    window.history.replaceState(null,"","/messages?thread=a");
    const initial = detail("a");
    let finish!: (value: unknown) => void;
    fetchMock.mockImplementationOnce(() => new Promise(resolve => { finish=resolve; })).mockResolvedValueOnce(response("a"));
    const { result } = renderHook(() => useConversationSelection("a", initial));
    await act(async () => { await new Promise(resolve => setTimeout(resolve,0)); });
    act(() => { for(let i=0;i<10;i++) result.current.revalidate(); });
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(result.current.detail).toBe(initial); expect(result.current.loading).toBe(false);
    expect(result.current.refreshing).toBe(true);
    await act(async () => finish(response("a")));
    await waitFor(() => expect(result.current.refreshing).toBe(false));
    expect(fetchMock).toHaveBeenCalledTimes(2); expect(markRead).not.toHaveBeenCalled();
  });

  it("keeps loaded content but fails the reply gate when focused refresh fails", async () => {
    window.history.replaceState(null,"","/messages?thread=a");
    const initial=detail("a");
    ...
    await waitFor(()=>expect(result.current.refreshError).toMatch(/before replying/));
    expect(result.current.detail).toBe(initial); expect(result.current.error).toBeNull();
    act(()=>result.current.revalidate()); await waitFor(()=>expect(result.current.refreshError).toBeNull());
  });
