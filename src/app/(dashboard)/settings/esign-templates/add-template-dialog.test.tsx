...
describe("AddTemplateDialog", () => {
  ...
  it("exposes an accessible max-xl PDF drop surface with browse and Dropbox choices", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Choose from Dropbox" }),
    ).toBeVisible();
  });
  ...
  it("accepts one dropped PDF and rejects multiple dropped files", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Add template" }));
    const source = screen.getByRole("group", { name: "PDF source" });
    const offer = new File(["%PDF-1.7"], "offer.pdf", {
  ...
  it("accepts an uploaded PDF exactly 40 MiB and rejects one byte over", () => {
    ...
    expect(
      screen.queryByText("The PDF must be 40 MB or smaller."),
    ).not.toBeInTheDocument();

    fireEvent.change(input, {
      target: { files: [sizedPdf("over.pdf", 40 * 1024 * 1024 + 1)] },
    });
    expect(screen.getByRole("alert")).toHaveTextContent(
      "The PDF must be 40 MB or smaller.",
    );
  });
  ...
  it("accepts a dropped PDF exactly 40 MiB and rejects one byte over", () => {
    ...
    fireEvent.drop(source, {
      dataTransfer: { files: [sizedPdf("exact-drop.pdf", 40 * 1024 * 1024)] },
    });
    expect(
      screen.getByText(/exact-drop\.pdf.*40\.0 MB.*Uploaded/),
    ).toBeVisible();

    fireEvent.drop(source, {
      dataTransfer: {
        files: [sizedPdf("over-drop.pdf", 40 * 1024 * 1024 + 1)],
      },
    });
    expect(screen.getByRole("alert")).toHaveTextContent(
      "The PDF must be 40 MB or smaller.",
  ...
  it("submits one PDF with exact roles and fixed merge labels", async () => {
    const createDraft = vi
      .fn()
      .mockResolvedValue({ ok: true, data: { templateId: "local-1" } });
    ...
    const file = new File(["%PDF-1.7"], "offer.pdf", {
      type: "application/pdf",
    });
  ...
  it("registers website templates with validation, provider error display, success refresh, and pressed mode state", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("alert")).toHaveTextContent(
        "Dropbox Sign Sender merge fields must match a supported contract field set",
      ),
    );
  ...
  it("accepts 80 emoji but rejects 81 using the UTF-16 title limit", () => {
    ...
    expect(
      screen.queryByText("Template names must be 160 characters or fewer."),
    ).not.toBeInTheDocument();
  ...
  it("allows exact case-distinct roles and rejects only exact trimmed duplicates", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Signer role 2"), {
      target: { value: " Seller " },
    });
    expect(screen.getByRole("alert")).toHaveTextContent(
      "Signer role names must be unique",
    );
    fireEvent.change(screen.getByLabelText("Signer role 2"), {
      target: { value: "seller" },
    });
...
function makeActions(
  overrides: Partial<TemplateLibraryActions> = {},
...
function renderDialog(actions: TemplateLibraryActions) {
  return render(
    <InitialEditorSessionProvider>
      <AddTemplateDialog actions={actions} />
    </InitialEditorSessionProvider>,
  );
}
