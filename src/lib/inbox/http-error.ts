...
export function inboxDatabaseError(error: unknown): InboxHttpError {
  ...
  if (code === "42501") {
    ...
    if (["INBOX_MEMBERSHIP_AMBIGUOUS_OR_MISSING", "INBOX_ORG_DENIED", "INBOX_REPLACEMENT_DENIED"].includes(String(message))) return new InboxHttpError(403);
  }
  if (code === "22023" && message === "INBOX_INVALID_WORKSET") return new InboxHttpError(400);
