...
function transportHarness(overrides: Partial<JitterTransportDependencies> = {}) {
  ...
  const dependencies: JitterTransportDependencies = {
    startCall: vi.fn(async () => ({ ok: true as const, data: { sessionRef: "session-1", batchId: "batch-1" } })),
    ...
    cancel: vi.fn(async () => ({ ok: true as const, data: { tornDown: true as const } })),
...
describe("JitterCallTransport", () => {
  ...
  it.each([
    [409, "operator_busy"],
    [422, "not_callable"],
  ])("maps a %s start envelope to failed without inventing a session to cancel", async (status, errorCode) => {
    ...
    const states: string[] = [];
    harness.transport.onStateChange((state) => states.push(state));
    await expect(harness.transport.start({ phoneE164: "+18165550123" })).rejects.toMatchObject({ name: errorCode });
  ...
  it("rejects an expired initial RTC token and cancels the provisioned session", async () => {
    ...
    const states: string[] = [];
    harness.transport.onStateChange((state) => states.push(state));
    await expect(harness.transport.start({ phoneE164: "+18165550123" })).rejects.toMatchObject({
      name: "rtc_token_expired",
    });
    expect(harness.dependencies.createRtcClient).not.toHaveBeenCalled();
    expect(harness.dependencies.cancel).toHaveBeenCalledWith("session-1", "failed");
    expect(states).toEqual(["connecting", "failed"]);
  });
  ...
  it("refreshes the short-lived token on Telnyx's expiry warning", async () => {
    const getToken = vi.fn()
      .mockResolvedValueOnce({
        ok: true,
        data: { rtcToken: "rtc-token-1", sipIdentity: "operator-1", expiresAt: "2026-08-21T20:05:00.000Z" },
      })
      .mockResolvedValueOnce({
        ok: true,
        data: { rtcToken: "rtc-token-2", sipIdentity: "operator-1", expiresAt: "2026-08-21T20:10:00.000Z" },
      });
    const harness = transportHarness({ getToken });
    await harness.transport.start({ phoneE164: "+18165550123" });
  ...
  it("captures duration at terminal intent instead of including slow cancel latency", async () => {
    let resolveCancel!: (value: { ok: true; data: { tornDown: true } }) => void;
    const cancel = vi.fn(() => new Promise<{ ok: true; data: { tornDown: true } }>((resolve) => {
    ...
    resolveCancel({ ok: true, data: { tornDown: true } });
  ...
  it("uses the authenticated same-origin beacon on pagehide and avoids duplicate cancellation", async () => {
    ...
    await flush();
    expect(sendCancelBeacon).toHaveBeenCalledWith("session-1", "abandoned");
    expect(harness.dependencies.cancel).not.toHaveBeenCalled();
    expect(harness.rtc.disconnect).toHaveBeenCalledTimes(1);
