...
const options = { orgId: "org", callId: "123", providerUserId: "456", recordingId: "segment", recordingKind: "admincallrecording" };
const event = (timestamp: number, url: string, patch = {}) => ({ payload: {
...
function setup(rows: unknown[], failed = false) {
  const fetcher = vi.fn<typeof fetch>().mockResolvedValue(Response.json(failed ? { message: "private" } : rows, { status: failed ? 500 : 200 }));
