/**
 * Minimal Handlebars-compatible template engine for SMS bodies.
 *
 * Supported syntax:
 *   - `{{variable}}`                      — substitution
 *   - `{{variable | fallback}}`           — substitution with fallback value
 ...
 *   - When a `| fallback` is provided, the fallback is used instead of `""`.
 ...
 *
 * History:
 *   - Originally in `src/lib/sequences/render.ts` (sequence-only).
 *   - Moved to `src/lib/templates/render.ts` to share across sequences,
 *     compose box, and the templates management UI.
 *   - Added `| fallback` pipe syntax (2026-04-29).
 */
...
/**
 * Matches both `{{variable}}` and `{{variable | fallback}}`.
 * Group 1 = variable name, Group 2 = fallback value (trimmed, may be undefined).
 */
...
export function renderTemplate(
  ...
  // Pass 2: substitute bare variables (with optional pipe fallback).
  result = result.replace(VARIABLE_REGEX, (_match, varName, fallback) => {
    const val = vars[varName];
    if (val === null || val === undefined || (typeof val === "string" && val.trim() === "")) {
      // Use fallback if provided, otherwise empty string.
