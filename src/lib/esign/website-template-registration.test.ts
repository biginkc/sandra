...
import { ESIGN_MERGE_FIELD_NAMES, ESIGN_NOVATION_FIELD_NAMES, ESIGN_NOVATION_TWO_SELLER_ROLES, ESIGN_RESIDENTIAL_FIELD_NAMES, type ProviderTemplateMetadata } from "./contracts";
import {
...
describe("Dropbox website eSign template registration", () => {
  ...
  it("registers a novation template with repeated sender fields", async () => {
    ...
    data.documents[0].customFields = [
      ...ESIGN_NOVATION_FIELD_NAMES.map((name) => field(name, { required: name !== "additional_terms" })),
  ...
  it("registers a novation template with a required second seller signature", async () => {
    const data = metadata();
    data.signerRoles = ESIGN_NOVATION_TWO_SELLER_ROLES.map((role) => ({ ...role }));
    data.documents[0].customFields = ESIGN_NOVATION_FIELD_NAMES.map((name) =>
      field(name, { required: name !== "additional_terms" }));
    data.formFields.push(field("seller_two_signature", {
      apiId: "seller-two-signature-api", type: "signature", required: true,
      signer: "2", assignedTo: "signer", signerRoleName: "Seller 2",
    }));
    mocks.getTemplate.mockResolvedValue(data);
    await expect(registerDropboxWebsiteTemplate({ orgId: "org-1", actorId: "user-1",
      providerTemplateId: "provider-template-1", name: "Novation packet, two sellers", documentType: "novation_agreement" }))
      .resolves.toMatchObject({ signerRoles: ESIGN_NOVATION_TWO_SELLER_ROLES });
  });
