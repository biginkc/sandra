...
describe("reviewed import dataset identity", () => {
  it("uploads the transformed rows the operator reviewed, including DNC, instead of the original vendor bytes", async () => {
    ...
    expect(uploadedBytes).toBe(
      serializeReviewedDataset(locked.rows, locked.headers),
    );
    expect(uploadedBytes).not.toBe(
      serializeReviewedDataset(original, originalHeaders),
    );

    const parsed = Papa.parse<Record<string, string>>(uploadedBytes, {
      header: true,
    });
  ...
  it("locks only true DNC rows and keeps SMS-only suppression reversible", () => {
    const rows = [
    ...
    const merged = mergeServerPreflight(local.preflight, {
      ...
      trueDncRows: [],
      smsSuppressedRows: [
        { rowIndex: 1, reasons: ["Existing contact opted out of SMS"] },
      ],
    });
    const locked = withGeneratedDncLocks({
      rows,
    ...
    expect(merged.dnc).toBe(1);
    expect(merged.smsSuppressed).toBe(1);
    expect(locked.rows.map((row) => row.__sandra_dnc_locked)).toEqual([
      "true",
      "",
    ]);
    expect(
      locked.rows.filter(
        (row, index) =>
          validateRow(row, locked.mapping, index).normalized
