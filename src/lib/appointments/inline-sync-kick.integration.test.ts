 ...
 *   3. `kickCalendarMutationSync` — the exact function every appointment
 *      server action now calls right after its RPC commits — claims and
 ...
 *
 * This exercises the REAL `fn_claim_calendar_mutations` claim RPC and the
...
type InlineKickTestRpcClient = {
  rpc(fn: "fn_book_appointment", args: BookAppointmentArgs): Promise<RpcResult<BookAppointmentRow>>;
  rpc(fn: "fn_reschedule_appointment", args: RescheduleArgs): Promise<RpcResult<RescheduleRow>>;
};

function asRpcClient(client: SupabaseClient<Database>): InlineKickTestRpcClient {
  return client as unknown as InlineKickTestRpcClient;
}

function bookAppointment(client: SupabaseClient<Database>, args: BookAppointmentArgs) {
...
type LedgerRow = { id: string; phase: string; result_reason: string | null; operation: string };
function ledgerRowsForTask(taskId: string): Promise<{ data: LedgerRow[] | null; error: unknown }> {
  return (db as unknown as {
    from(table: "task_calendar_mutations"): {
      select(columns: string): {
        eq(column: "source_task_id", value: string): Promise<{ data: LedgerRow[] | null; error: unknown }>;
      };
    };
  })
    .from("task_calendar_mutations")
    .select("id, phase, result_reason, operation")
...
describe("kickCalendarMutationSync — real DB: booking lock cleared inline instead of waiting on the cron sweep", () => {
  it("reschedule is locked out right after booking, and kickCalendarMutationSync clears it so the retry succeeds", async () => {
    ...
    // Step 1: book — fn_book_appointment opens the create ledger row
    // (phase='pending') in the same transaction as the task.
    const { data: booked, error: bookErr } = await bookAppointment(booker.client, {
      p_org: BMH_ORG_ID,
    ...
    expect(ledgerBefore.data![0]).toMatchObject({ operation: "create", phase: "pending" });
    ...
    const { data: lockedReschedule, error: lockedErr } = await rescheduleAppointment(booker.client, {
      p_task: taskId,
    ...
    await kickCalendarMutationSync(db);
    ...
    // above now succeeds.
    const { data: reschedule, error: rescheduleErr } = await rescheduleAppointment(booker.client, {
      p_task: taskId,
      p_new_start: newStart,
      p_new_end: newEnd,
      p_timezone: "America/Chicago",
