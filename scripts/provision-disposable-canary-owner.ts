...
const runEnvironment = process.env.E2E_RUN_SLUG && process.env.E2E_TEST_USER_EMAIL && process.env.E2E_TEST_USER_PASSWORD
  ? ensureE2ERunEnvironment()
  : process.env.GITHUB_ACTIONS === "true"
    ? createGitHubRunEnvironment()
    : ensureE2ERunEnvironment();
const identity = identityForPrincipal(runEnvironment);
