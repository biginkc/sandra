...
try {
  ...
  psql(`
    ...
    insert into csv_imports(id,org_id,storage_path,source,market,county_id,dataset_sha256,review_contract_sha256,dataset_version,total_rows) values
      ('${importA}','${orgA}','${orgA}/a.csv','dealmachine','Kansas City','${county}','${shaA}','${shaB}',1,2),
    ...
    insert into jobs(id,org_id,type,status,related_import_id,total_items) values
      ('${jobA}','${orgA}','csv_import','failed','${importA}',2),
  ...
  psql(`${service}
    ...
    values ('${jobA}','${orgA}','${importA}','${orgA}/a.csv','dealmachine','Kansas City','${county}','{}','${shaA}','${shaB}',1,2);
    insert into job_items(job_id,status,source_row_index) values ('${jobA}','success',0),('${jobA}','error',1);
  `);
  ...
  equal(psql(`select status||':'||processed_items||':'||succeeded_items||':'||failed_items from jobs where id='${jobA}'`), "partial:2:1:1", "failure checkpoint counts");
  ...
  equal(psql(`select count(*) from properties where org_id='${orgA}'`), "1", "one business row after replay");
