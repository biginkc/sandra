...
## Pre-merge fixes applied (review round 2)

- `70a7d29` — replaced first-touch random conversation ids with deterministic contact+property ids, removed the abandoned DB thread-registry migration from this branch, and restored the race regression coverage.
- `a95299d` — changed pending inbound retries to resume downstream automation instead of short-circuiting on duplicate inbound rows, added replay guards for owner notifications, and stamped AI replies with `metadata.in_reply_to_external_id`.
- `037c4ab` — tightened migration `062` so rollout fails loudly before the inbound uniqueness index if duplicate inbound `(provider, external_id)` groups already exist.
- `8fa9371` — moved STOP/DNC/HELP replay idempotency into the consent layer with schema-backed dedupe keyed by source external id, while still reapplying the safe opt-out/contact suppression path on replay.
