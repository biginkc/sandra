-- Assignment is actionable only after a property has been promoted out of the
-- Prospects pool. Keep the production-proven narrow inbox query unchanged, but
-- make both assignment chips use Sandra's canonical lead boundary:
-- property_status is present and is not 'prospect'. Propertyless conversations
-- and ordinary prospects remain visible in All and the other operational
-- filters, but never inflate Mine or No owner.
