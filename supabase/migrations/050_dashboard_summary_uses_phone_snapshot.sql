-- 050_dashboard_summary_uses_phone_snapshot.sql
--
-- Replaces the live skip_trace_cache join that computed v_traced_num /
-- v_traced_den with a read from metric_snapshots (metric_key =
-- 'phone_coverage').  The daily cron at /api/cron/phone-coverage-snapshot
-- keeps the snapshot fresh.  Everything else in the function body is
-- identical to migration 048.
--
-- A backfill INSERT at the end of this migration seeds today's row so
-- the dashboard donut is correct immediately on deploy without waiting
-- for the first cron run.
