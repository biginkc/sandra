...
async function makeClassificationRun(
  ...
  await db.query(
    `insert into public.sms_classification_runs
       (id, org_id, property_id, conversation_id, source_inbound_message_id, provider, model, schema_version, policy_version, state_hash, state_version, resolved_outcome)
     values ($1, $2, $3, $4, $5, $6, 'jev-1.13.0', 2, '2026-09-21-blocker-3', $7, 1, $8)`,
    [
      id,
...
async function proposeDeferred(fixture: Fixture, classificationRunId: string, disposition = "opted_out") {
  ...
  const { rows } = await db.query(
    ...
    [fixture.propertyId, fixture.conversationId, fixture.messageId, classificationRunId, disposition, "Jev classified below threshold", 0],
  );
...
async function proposeDnc(fixture: Fixture, classificationRunId: string) {
  ...
  const { rows } = await db.query(
    ...
    [fixture.propertyId, fixture.conversationId, fixture.messageId, classificationRunId, "Jev classified dnc", 0],
  );
  await db.query("reset role");
  return rows[0].result;
}
