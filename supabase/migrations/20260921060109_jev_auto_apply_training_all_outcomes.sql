-- Root review of 3e4ee3b1 (jev-root-round12-review.md), finding 2:
--
-- fn_auto_apply_jev_lead_decision only checked is_training inside the
-- nurture branch. A training prospect (status='prospect', is_training)
-- could still take the new_lead branch and be auto-promoted — a real
-- customer-state effect (status/qualified_by/qualified_at) on a
-- server-only fictional practice lead.
--
-- Fixed: the training check now runs once, before the outcome branch,
-- rejecting BOTH new_lead and nurture with the same error, no effect and
-- no audit row either way. DNC-locked stays a new_lead-only check
-- (untouched — a training property is never real DNC data).
