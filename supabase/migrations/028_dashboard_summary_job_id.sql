-- 028_dashboard_summary_job_id.sql
--
-- Recreates `dashboard_summary()` with `job_id` added to the
-- `import_done` and `skip_trace_done` activity-feed events. Without it,
-- the activity feed's click-through can only land on /jobs (the list);
-- with it, the click deep-links to /jobs/<id> (the detail page that
-- shipped in PR #54).
--
-- Mechanically this is migration 025 with one extra key in the events
-- jsonb_build_object on lines 242-247. Everything else is identical;
-- `create or replace function` swaps the body atomically.
...
begin
  ...
  with raw_events as (
    ...
    -- ⬇ NEW: include job_id so the dashboard activity feed can deep-link
    --       to /jobs/<id> on click instead of /jobs (the list).
    (select jsonb_build_object(
