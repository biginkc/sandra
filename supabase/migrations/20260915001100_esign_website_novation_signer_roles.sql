-- Preserve provider signer roles when registering website templates.
-- Repair novation registrations stored with only Seller and Buyer.
begin;
...
begin
  ...
  insert into public.esign_templates (
    ...
    p_provider_metadata -> 'signerRoles',
  ...
  set name = btrim(p_name),
      ...
      signer_roles = p_provider_metadata -> 'signerRoles',
...
set signer_roles = template.provider_metadata -> 'signerRoles',
    updated_at = now()
where template.template_origin = 'dropbox_website'
  and template.deleted_at is null
  and cardinality(template.merge_field_names) = 26
  and jsonb_typeof(template.provider_metadata -> 'signerRoles') = 'array'
  and public.esign_website_template_metadata_is_valid(
    template.sign_template_id, template.provider_account_id, template.provider_metadata
  )
  and template.signer_roles is distinct from template.provider_metadata -> 'signerRoles';
