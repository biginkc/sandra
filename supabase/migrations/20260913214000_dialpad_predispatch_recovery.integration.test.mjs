...
try {
  ...
  await client.query(readFileSync(new URL('./20260913214000_dialpad_predispatch_recovery.sql',import.meta.url),'utf8'));
  const recover=async(scope=org,limit=20)=>invoke('fn_recover_dialpad_pre_dispatch',scope,limit);
  const enrollment=await enroll();
  const unprepared=await makeIntent();
  assert.deepEqual(await recover(),{recovered:1,resumed:0});
  assert.equal((await invoke('fn_dispatch_dialpad_intent',unprepared)).dispatched,false);
  const paused=await makeIntent();await invoke('fn_prepare_dialpad_sequence_pause',paused);
  assert.deepEqual(await recover(),{recovered:1,resumed:1});
  assert.equal((await client.query('select status from sequence_enrollments where id=$1',[enrollment])).rows[0].status,'active');
  assert.deepEqual(await recover(),{recovered:0,resumed:0});
  const fresh=await makeIntent(rep,0);
  assert.deepEqual(await recover(),{recovered:0,resumed:0});
  await invoke('fn_release_dialpad_start',fresh,null);
  const ambiguous=await makeIntent();await invoke('fn_prepare_dialpad_sequence_pause',ambiguous);await invoke('fn_dispatch_dialpad_intent',ambiguous);
  assert.deepEqual(await recover(),{recovered:0,resumed:0});
  assert.equal((await client.query('select status from dialpad_voice_intents where id=$1',[ambiguous])).rows[0].status,'initiation_unconfirmed');
  assert.equal((await client.query('select status from sequence_enrollments where id=$1',[enrollment])).rows[0].status,'paused');
  await invoke('fn_release_dialpad_start',ambiguous,422);
  const locked=await makeIntent();await invoke('fn_prepare_dialpad_sequence_pause',locked);
  const contender=new pg.Client({host:dir,user:'postgres',database:'postgres'});await contender.connect();
  try {
    await contender.query('begin');
    await contender.query('select id from dialpad_voice_intents where id=$1 for update',[locked]);
    assert.deepEqual(await recover(),{recovered:0,resumed:0});
    assert.equal((await contender.query('select fn_dispatch_dialpad_intent($1) result',[locked])).rows[0].result.dispatched,true);
    await contender.query('commit');
    assert.deepEqual(await recover(),{recovered:0,resumed:0});
  } finally { await contender.query('rollback');await contender.end(); }
  assert.deepEqual(await recover(randomUUID()),{recovered:0,resumed:0});
  await assert.rejects(recover(org,101),/DIALPAD_RECOVERY_INVALID_INPUT/);
  await client.query('set role authenticated');await assert.rejects(client.query('select fn_recover_dialpad_pre_dispatch($1)',[org]),/permission denied/);await client.query('reset role');
  console.log('PASS: bounded predispatch recovery restores owned pause; fresh/dispatched/other-org untouched; idempotent; dispatch cannot claim recovered reservation.');
} finally {
