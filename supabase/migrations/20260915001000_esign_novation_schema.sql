...
returns boolean language sql immutable set search_path = public, pg_temp as $$
  select coalesce((select array_agg(field order by field) from unnest(p_fields) field)
    in (array['closing_date', 'earnest_money', 'offer_price', 'property_address', 'seller_name']::text[], array['additional_terms', 'buyer_name', 'cash_balance', 'closing_date', 'earnest_money', 'earnest_money_holder', 'legal_description', 'offer_price', 'property_address', 'property_city', 'property_state', 'property_zip', 'seller_name']::text[], array['acceptance_date', 'access_days_per_week', 'access_hours_per_visit', 'additional_terms', 'agreement_date', 'attorney_in_fact', 'buyer_email', 'buyer_name', 'buyer_phone', 'closing_agent_address', 'closing_agent_name', 'closing_agent_phone', 'closing_date', 'due_diligence_days', 'earnest_money', 'earnest_money_holder', 'legal_description', 'offer_expiration', 'offer_price', 'property_address', 'property_state', 'release_date', 'seller_closing_cost_cap', 'seller_email', 'seller_name', 'seller_phone']::text[]), false);
$$;
...
as $$
  ...
  required_signature_fields as (
    ...
    where field ->> 'assignedTo' = 'signer'
      and btrim(coalesce(field ->> 'apiId', '')) <> ''
      and field ->> 'signerRoleName' in ('Seller', 'Seller 2', 'Buyer')
  )
  select coalesce(btrim(coalesce(p_provider_template_id, '')) <> ''
    ...
    and (p_metadata -> 'signerRoles' =
      jsonb_build_array(
        ...
        cardinality(public.esign_website_sender_field_names(p_metadata)) = 26
        and p_metadata -> 'signerRoles' = jsonb_build_array(
          jsonb_build_object('name', 'Seller', 'order', 0),
          jsonb_build_object('name', 'Seller 2', 'order', 1),
          jsonb_build_object('name', 'Buyer', 'order', 2)
        )
      ))
    ...
    and (cardinality(public.esign_website_sender_field_names(p_metadata)) = 26
    ...
    and (cardinality(public.esign_website_sender_field_names(p_metadata)) <> 26 or not exists (
      select 1 from sender_custom_fields
      where field -> 'required' is distinct from to_jsonb(field ->> 'name' <> 'additional_terms')
    ))
    and not exists (
      ...
      where field ->> 'assignedTo' is distinct from 'signer'
        ...
        or not exists (
          select 1 from jsonb_array_elements(case
            when jsonb_typeof(p_metadata -> 'signerRoles') = 'array'
              then p_metadata -> 'signerRoles' else '[]'::jsonb end) role(value)
          where role.value ->> 'name' = role_name
        )
    )
    and not exists (
      select 1 from jsonb_array_elements(case
        when jsonb_typeof(p_metadata -> 'signerRoles') = 'array'
          then p_metadata -> 'signerRoles' else '[]'::jsonb end) role(value)
      where not exists (
        select 1 from required_signature_fields
        where role_name = role.value ->> 'name'
      )
    ), false);
$$;
