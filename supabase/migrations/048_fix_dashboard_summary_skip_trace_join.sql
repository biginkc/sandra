-- 048_fix_dashboard_summary_skip_trace_join.sql
--
-- The skip_trace_cache.address_normalized column uses a pipe-delimited key
-- built by normalizeAddress() (e.g. "123 main st|olathe|ks|66061"), while
-- properties.address_normalized stores a freeform comma-separated string
-- (e.g. "123 Main St, Olathe KS 66061"). The two formats never match, so
-- the join in dashboard_summary() always returned 0 traced properties.
--
-- Fix: reconstruct the cache key inline from properties' individual columns
-- (address, city, state, zip) using the same logic as normalizeAddress().
-- Everything else in the function body is identical to migration 028.
...
begin
  ...
  -- Reconstruct the pipe-delimited cache key from individual address columns
  -- (street|city|state[|zip]) to match how normalizeAddress() writes the key.
  select count(distinct p.id) into v_traced_num
  from properties p
  join skip_trace_cache c on c.address_normalized = (
    lower(trim(p.address)) || '|' || lower(trim(p.city)) || '|' || lower(trim(p.state)) ||
    case when p.zip is not null and trim(p.zip) != '' then '|' || lower(trim(p.zip)) else '' end
  )
  where p.deleted_at is null
