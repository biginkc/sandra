-- Calculator owner access follows the authenticated My Leads owner capability.
-- Keep the organization workflow flag for non-owners while allowing an active
-- owner to use calculators without the Acquisitions designation. This is
-- additive so deployments that already applied the original calculator
-- migration can update only the save boundary.
