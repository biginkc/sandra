...
-- Preserve prior partial PDF evidence without presenting it as the canonical file.
create table if not exists public.esign_signed_artifact_archives (
  ...
  reason text not null check (reason = 'superseded_partial_pdf'),
  archived_at timestamptz not null default now(),
  foreign key (request_id, org_id) references public.esign_requests(id, org_id),
  unique (org_id, request_id, storage_path)
);
alter table public.esign_signed_artifact_archives enable row level security;
revoke all on public.esign_signed_artifact_archives from public, anon, authenticated, service_role;
grant select, insert on public.esign_signed_artifact_archives to service_role;

alter table public.lead_files drop constraint if exists lead_files_opaque_path_check;
alter table public.lead_files add constraint lead_files_opaque_path_check check (
    ...
    and split_part(storage_path, '/', 5) in ('signed.pdf', 'signed-final.pdf')
  );
...
begin
  ...
  where receipt.id = p_receipt_id
    ...
    and receipt.event_type = 'signature_request_all_signed'
    and receipt.provider_event_at is not null
  ...
  for update;
  if not found then
    raise exception 'active all-signed webhook receipt lease not found'
      using errcode = 'P0002';
  end if;
  -- Notification timestamps change on provider resend. Completion is the last
  -- authoritative signature time reconciled before artifact linkage.
  perform 1 from public.esign_request_signers signer
  where signer.org_id = p_org_id and signer.request_id = p_request_id
  for update;
  if not found or exists (
    select 1 from public.esign_request_signers signer
    where signer.org_id = p_org_id and signer.request_id = p_request_id
      and (signer.status is distinct from 'signed' or signer.signed_at is null)
  ) then
    raise exception 'authoritatively completed signer set required' using errcode = '23514';
  end if;
  select max(signer.signed_at) into v_completed_at
  from public.esign_request_signers signer
  where signer.org_id = p_org_id and signer.request_id = p_request_id;
  v_expected_path := p_org_id::text || '/' || v_request.property_id::text
    || '/esign/' || p_request_id::text || '/signed-final.pdf';
  v_legacy_path := p_org_id::text || '/' || v_request.property_id::text
    || '/esign/' || p_request_id::text || '/signed.pdf';
  if v_request.signed_pdf_path is not null
     and v_request.signed_pdf_path not in (v_expected_path, v_legacy_path) then
    raise exception 'conflicting signed PDF path exists' using errcode = '23514';
  end if;
  ...
  if p_lead_file_id is null
     or p_storage_bucket is distinct from 'lead-files'
     or p_storage_path is distinct from v_expected_path
     or p_content_type is distinct from 'application/pdf'
     or p_size_bytes is null
     or p_size_bytes <= 0
     or p_size_bytes > 41943040
     or p_lead_event_type is distinct from 'esign_signed_pdf_ready'
  ...
  if found then
    if v_existing.property_id <> v_request.property_id
       or v_existing.storage_bucket <> 'lead-files'
       or v_existing.file_name <> v_expected_name
       or v_existing.content_type <> 'application/pdf'
       or v_existing.storage_path not in (v_expected_path, v_legacy_path) then
      raise exception 'conflicting signed PDF linkage exists' using errcode = '23514';
    end if;
    if v_existing.storage_path = v_legacy_path then
      if v_request.signed_pdf_path is distinct from v_legacy_path then
        raise exception 'legacy signed PDF pointer changed' using errcode = '23514';
      end if;
      insert into public.esign_signed_artifact_archives
        (org_id, request_id, lead_file_id, storage_path, metadata_snapshot, reason)
      values (p_org_id, p_request_id, v_existing.id, v_legacy_path,
        to_jsonb(v_existing), 'superseded_partial_pdf');
      update public.lead_files
      set storage_path = v_expected_path, size_bytes = p_size_bytes
      where id = v_existing.id and org_id = p_org_id
        and storage_path = v_legacy_path;
      v_outcome := 'applied';
    else
      if v_existing.size_bytes <> p_size_bytes then
        raise exception 'conflicting signed PDF linkage exists' using errcode = '23514';
      end if;
      v_outcome := 'already_linked';
    end if;
  else
    -- A historical pointer may exist even when metadata linkage failed.
    if v_request.signed_pdf_path = v_legacy_path then
      insert into public.esign_signed_artifact_archives
        (org_id, request_id, lead_file_id, storage_path, metadata_snapshot, reason)
      values (p_org_id, p_request_id, null, v_legacy_path,
        jsonb_build_object('signed_pdf_path', v_legacy_path,
          'lead_file_missing', true), 'superseded_partial_pdf');
    end if;
  ...
  set signed_pdf_path = v_expected_path,
      completed_at = case when signed_pdf_path is distinct from v_expected_path
        then v_completed_at else completed_at end,
...
begin
  ...
  if v_receipt.processing_status in ('processed', 'ignored') and not (
    v_receipt.event_type = 'signature_request_all_signed'
    and v_request_id is not null
    and (v_receipt.esign_request_id is null or v_receipt.esign_request_id = v_request_id)
    and exists (
      select 1 from public.esign_requests request
      where request.id = v_request_id and request.org_id = p_org_id
        and request.sign_request_id = p_sign_request_id
        and request.delivery_state = 'sent'
        and request.status not in ('declined', 'voided')
        and (request.signed_pdf_path is null or request.signed_pdf_path =
          p_org_id::text || '/' || request.property_id::text
            || '/esign/' || request.id::text || '/signed.pdf')
    )
  ) then
...
as $$
begin
  raise exception 'Final PDF requires verified all-signed receipt lease' using errcode = '55000';
end;
$$;

comment on table public.esign_signed_artifact_archives is
  'Service-only immutable metadata evidence for superseded partial PDFs. Referenced storage objects are preserved.';
comment on function public.link_esign_signed_artifact(
  uuid, uuid, uuid, uuid, uuid, text, text, text, bigint, text, jsonb
) is 'Links only the immutable signed-final.pdf after an active verified all_signed lease and existing storage object; preserves prior partial PDF evidence atomically.';
...
) is 'Service-role verified lifecycle transition. Only all_signed may finalize; downloadable is never proof of completion.';
comment on function public.reconcile_esign_completed_signed_artifact(
  uuid, uuid, uuid, text, text, text, bigint, text, jsonb
) is 'Disabled legacy repair contract: a verified all_signed receipt lease is required for final PDF linkage.';
