...
begin
  ...
  select
    ...
    -- This projection is the lifecycle lock, not merely the property flag.
    -- Contact-only appointments and appointments linked to a non-homeowner
    -- contact must remain read-only when that exact contact is permanently
    -- DNC. NULL is reserved for a truly personal appointment with neither a
    -- property nor a contact.
    case
      when g.related_property_id is null and g.contact_id is null then null
      else coalesce(p.is_dnc_locked, false) or coalesce(c.do_not_contact, false)
    end,
