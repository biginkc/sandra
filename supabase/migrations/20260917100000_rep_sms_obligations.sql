...
-- Called by the two acquisition attempt RPCs and the provider reconciliation
-- trigger. There is no data backfill: old attempts remain old history, while
-- each newly observed no-answer becomes durable at the attempt boundary.
...
begin
  ...
  if not found or v_property.deleted_at is not null or v_property.is_dnc_locked
    ...
    where s.id=v_o.sender_assignment_id and s.org_id=p_org_id and s.user_id=p_actor_id
      and s.active and s.grant_status='active' and s.revoked_at is null) then
...
begin
  ...
  select * into v_o from public.rep_sms_obligations where id=p_obligation_id for update;
  if not found or v_o.state<>'sending' or v_o.claim_state<>'claimed' or v_o.claim_token is distinct from p_claim_token
...
begin
  ...
  if p_state='delivery_failed' and nullif(btrim(p_provider_error),'') is null then raise exception 'FAILURE_REQUIRES_REASON' using errcode='22023'; end if;
  update public.rep_sms_obligations set state=p_state,provider_status=coalesce(nullif(btrim(p_provider_status),''),provider_status),
    provider_error=coalesce(nullif(btrim(p_provider_error),''),provider_error),last_error=coalesce(nullif(btrim(p_provider_error),''),last_error),
    ...
    where id=v_o.id;
  insert into public.rep_sms_obligation_audit(org_id,obligation_id,actor_kind,action,from_state,to_state,reason,metadata)
    values(v_o.org_id,v_o.id,'service','provider_delivery',v_o.state,p_state,p_provider_error,coalesce(p_metadata,'{}'::jsonb));
