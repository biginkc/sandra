-- Sandra Dispo is a human-review queue, not a historical property-outcome
-- bucket. Admit conversations with a pending AI review even when their latest
-- message is older than the ordinary inbox cutoff, without widening the normal
-- recent-message scan into a full-history OR predicate.
...
as $$
  ...
  eligible as materialized (
    -- Normal inbox branch: remains bounded by the caller's recent cutoff.
    select
      m.id,
      m.org_id,
      m.conversation_id,
    ...
    -- Pending-review branch: the conversation index bounds this lookup to the
    -- small review workset. The mutually-exclusive date predicate prevents
    -- duplicate recent messages and avoids a whole-table OR scan.
    select
      m.id,
      m.org_id,
      m.conversation_id,
  ...
  ranked as materialized (
    select
      e.*,
      row_number() over (
        partition by e.org_id, e.conversation_id
  ...
  grouped as materialized (
    select
      ...
      bool_or(e.is_recent) as has_recent,
