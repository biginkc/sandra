...
--
-- The prior design split the Jev auto-apply into two separate calls from
-- dispatch.ts: (1) a TS-side property mutation (setOutreachDispoNurture /
-- qualifyProperty), which itself bumps properties.decision_context_revision
-- via trg_properties_bump_decision_context_revision, then (2) a SEPARATE
-- call to fn_auto_apply_jev_lead_decision passing the PRE-mutation
-- evaluationRevision as p_expected_revision. By the time step (2) ran, the
-- property's live revision had already moved because of step (1)'s own
-- write — so fn_auto_apply_jev_lead_decision's own STALE_DECISION_CONTEXT
-- check rejected EVERY normal successful auto-apply, and
-- autoApplyJevLeadDecision (a void helper) swallowed that rejection via
-- reportError, leaving dispatch.ts to report "auto_closed" with no actual
-- audit row ever created.
--
-- Fix: fold the effect (the nurture/new_lead property write), the revision
-- check, and the audit insert into ONE function, ONE transaction, in that
-- order — revision checked against the row's state BEFORE any write this
-- call makes, effect applied only if the check passes, audit row inserted
-- only if the effect succeeded. Root: "Do NOT merely add 1 or re-read/
-- rebase revision: that would bless intervening work" — this does neither;
-- the check is against the untouched pre-effect value, exactly as
-- classifyForDispatch captured it before Jev was even called.
--
-- dispatch.ts's TWO callers (jev_nurture, jev_promote_new_lead) now call
-- THIS function directly instead of qualifyProperty/setOutreachDispoNurture
-- + autoApplyJevLeadDecision. qualifyProperty/setOutreachDispoNurture
-- themselves are UNCHANGED and still used by every other (non-Jev-
-- automatic-apply) caller — manual qualify, the legacy Haiku qualifier,
-- deferred/below-threshold Jev dispositions, etc.
