-- MANUAL ONLY: emergency forward rollback for
-- 20260828022800_sms_inbox_ai_disposition_review_queue_timeout_recovery.sql.
--
-- This file is deliberately outside supabase/migrations and is never applied
-- automatically. If the recovery still exceeds four seconds twice under the
-- authenticated production probe, or /messages still returns a database error,
-- copy this file byte-for-byte to a NEWER timestamped migration, review it,
-- merge it through the guarded migration workflow, and verify all/unread/
-- needs_outcome. It restores the exact 20260817110000 narrow inbox function.
--
-- Tradeoff: Sandra Dispo's pending-review queue is temporarily unavailable,
-- including old pending reviews. Ordinary Messages access is restored. No
-- message, contact, property, or review rows are changed.
...
select pg_advisory_xact_lock(
  hashtextextended('sms-inbox-thread-page-snapshot-v1', 0)
);
...
begin
  ...
  if position('pending_reviews as materialized' in v_definition) = 0 then
    raise exception 'SMS_INBOX_ROLLBACK_REFUSED_UNEXPECTED_FUNCTION_SHAPE';
  end if;
end;
$$;

-- The first paginated inbox function still carried every message body and
-- detailed AI field through full-window classification. On Sandra's real
-- production history that wide materialized workset exceeded the statement
-- timeout even though only 200 rows were returned.
--
-- Keep the authoritative single-statement snapshot, but make the full-window
-- workset narrow. Hydrate body text and detailed AI delivery fields only after
-- the requested page has been selected.
...
as $$
  ...
  conversation_ambiguities as (
    select case
      -- An authenticated caller with zero or one active tenant cannot observe
      -- a cross-tenant conversation collision. Avoid a second all-message
      -- scan on the normal Sandra operator path.
  ...
  counts as (
    select
      count(*) filter (where not p_hide_noise or not c.is_noise)::integer as all_count,
      count(*) filter (where (not p_hide_noise or not c.is_noise) and p_assignee_id is not null and c.assigned_user_id = p_assignee_id)::integer as mine_count,
      count(*) filter (where (not p_hide_noise or not c.is_noise) and p_assignee_id is not null and c.assigned_user_id is null)::integer as unassigned_count,
      count(*) filter (where (not p_hide_noise or not c.is_noise) and c.unread_count > 0)::integer as unread_count,
      count(*) filter (where (not p_hide_noise or not c.is_noise) and c.ai_responder_status = 'escalated')::integer as escalated_count,
      count(*) filter (where (not p_hide_noise or not c.is_noise) and c.outreach_dispo is not null)::integer as dispo_count,
      count(*) filter (where (not p_hide_noise or not c.is_noise) and c.needs_outcome)::integer as needs_outcome_count
    from classified c
  ),
  active_unhidden as materialized (
    ...
    where case b.active_filter
      when 'mine' then p_assignee_id is not null and c.assigned_user_id = p_assignee_id
      when 'unassigned' then p_assignee_id is not null and c.assigned_user_id is null
      when 'unread' then c.unread_count > 0 or c.conversation_id = p_include_thread_id
      when 'escalated' then c.ai_responder_status = 'escalated'
      when 'dispo' then c.outreach_dispo is not null
      when 'needs_outcome' then c.needs_outcome
      else true
    end
  ),
  active_filtered as materialized (
    ...
    where not p_hide_noise or not active.is_noise
  ),
  page_meta as (
    select
...
comment on function public.sms_inbox_thread_page_snapshot(timestamptz, text, uuid, uuid, boolean, integer, integer) is
  'RLS-scoped SMS inbox: authoritative full-window counts with narrow classification and page-only detail hydration.';
...
begin
  ...
  if position('pending_reviews as materialized' in v_definition) <> 0
     or position('eligible as materialized' in v_definition) = 0 then
    raise exception 'SMS_INBOX_ROLLBACK_VERIFICATION_FAILED';
  end if;

  if not has_function_privilege(
       'authenticated',
       'public.sms_inbox_thread_page_snapshot(timestamptz,text,uuid,uuid,boolean,integer,integer)',
       'execute'
     )
    ...
    raise exception 'SMS_INBOX_ROLLBACK_ACCESS_VERIFICATION_FAILED';
  end if;
end;
$$;
